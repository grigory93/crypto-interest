...
g_currencies = rbindlist(list(
  ...
  list(currency='MATIC',
       currency_join = 'MATIC',
       network='Polygon',
       desc='Polygon (MATIC) is an Ethereum token that powers the Polygon Network, a scaling solution for Ethereum. Polygon aims to provide faster and cheaper transactions on Ethereum using Layer 2 sidechains, which are blockchains that run alongside the Ethereum main chain. Users can deposit Ethereum tokens to a Polygon smart contract, interact with them within Polygon, and then later withdraw them back to the Ethereum main chain. The MATIC token is used to pay transaction fees and participate in proof-of-stake consensus.',
       max_value=20000.,
...
getCryptoComData <- function(platform = "Crypto.com") {
  ...
  cryptocom_data = transpose(data.table(
      ...
      BTC =        c('BTC', Inf, 3., 0.0005),
      ETH =        c('ETH', Inf, 3., 0.005),
      ...
      UNI = c('UNI', Inf, 2., 1.),
      BAT = c('BAT', Inf, 2., 14.),
      DOT = c('DOT', Inf, 11., 0.1),
      ADA = c('ADA', Inf, 2., 0.5),
      LUNA = c('LUNA', Inf, 4., 0.0015),
      SOL = c('SOL', Inf, 3., 0.0005),
      USDT = c('USDT', Inf, 6., 25.),
      USDC = c('USDC', Inf, 6., 25.),
      PAX = c('PAX', Inf, 6., 25.),
